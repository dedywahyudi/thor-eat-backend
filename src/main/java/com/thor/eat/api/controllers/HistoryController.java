...
/**
 * The history REST controller. Is effectively thread safe.
 *
 * @author TCSDEVELOPER
 * @version 1.0
 */
...
public class HistoryController {
    ...
    protected void checkConfiguration() {
        Helper.checkConfigNotNull(historyService, "historyService");
    }
    ...
    public History get(@PathVariable Long id) throws ThorEarlyException {
    ...
    @Transactional
    public History create(@Validated({ValidationGroup.Create.class, Default.class}) @RequestBody History entity)
            throws ThorEarlyException  {
        return historyService.create(entity);
    }
    ...
    @Transactional
    public History update(@PathVariable Long id, @Validated(Default.class) @RequestBody History entity)
            throws ThorEarlyException  {
        return historyService.update(id, entity);
    }
    ...
    public SearchResult<History> search(@ModelAttribute HistorySearchCriteria criteria,
                                      @ModelAttribute Paging paging) throws ThorEarlyException  {
        if (paging.getSortColumn() == null) {
            paging.setSortColumn("modifiedDate desc");
        }
        return historyService.search(criteria, paging);
    }
}
