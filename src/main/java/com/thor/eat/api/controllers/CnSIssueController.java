...
/**
 * The cns issue REST controller. Is effectively thread safe.
 *
 * @author TCSDEVELOPER
 * @version 1.0
 */
@RestController
@RequestMapping("/cnSIssues")
@NoArgsConstructor
public class CnSIssueController {
    ...
    protected void checkConfiguration() {
        Helper.checkConfigNotNull(cnSIssueService, "cnSIssueService");
    }
    ...
    @Transactional
    public CnSIssue get(@PathVariable Long id) throws ThorEarlyException {
        return cnSIssueService.markRead(id);
    }
    ...
    @Transactional
    public CnSIssue create(@Validated({ValidationGroup.Create.class, Default.class}) @RequestBody CnSIssue entity)
            throws ThorEarlyException  {
        return cnSIssueService.create(entity);
    }
    ...
    @Transactional
    public CnSIssue update(@PathVariable Long id, @Validated(Default.class) @RequestBody CnSIssue entity)
            throws ThorEarlyException  {
        return cnSIssueService.update(id, entity);
    }
    ...
    public SearchResult<CnSIssue> search(@ModelAttribute CnSIssueSearchCriteria criteria,
                                      @ModelAttribute Paging paging) throws ThorEarlyException  {
        if (paging.getSortColumn() == null) {
            paging.setSortColumn("priority asc,createdDate desc");
        }

        SearchResult<CnSIssue> result = cnSIssueService.search(criteria, paging);
        for (CnSIssue entity : result.getItems()) {
            cnSIssueService.fillDivisionName(entity);
        }
        return result;
    }
}
